...
class DataConfig:
    ...
    # Optional override for the LeRobot video backend (e.g. "torchcodec", "pyav", "none").
...
class LeRobotSoArmDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[soarm_policy.SoArmInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[soarm_policy.SoArmOutputs()],
        )

        # My SoArm actions dim is 9, first 5 are joints that should be converted to delta actions.
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        name="pi0_soarm_fast_lora",

        # Action dim should be aligned with the robot and dataset, 9 for SoArm
